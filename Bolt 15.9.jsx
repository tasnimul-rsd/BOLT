#target aftereffects
#targetengine "bolt_15_9_hero_authority_r1"

/*
Bolt v15.9 Production
...
(function BoltPanel(thisObj) {
    var VERSION = "15.9";
    var BUILD_ID = "15.9-hero-authority-r1";
    var SCRIPT_NAME = "Bolt";
    var SETTINGS_SECTION = "Bolt";
    var DEVELOPER_NAME = "Tasnimul Hasan Malik";
    var DEVELOPER_ROLE = "Designer & Animator";

    // Official Bolt update source. The repository can keep versioned files
    // such as "Bolt 15.9.jsx"; Bolt discovers the highest numeric version.
    var BOLT_UPDATE_PAGE_URL = "https://github.com/tasnimul-rsd/BOLT";
    ...
    var state = {
        ...
        // True only when the user explicitly presses Project > Main comp > Use.
        // Analyze/Organize must never overwrite an explicit Hero choice.
        heroCompManual: false,
        scrollAreas: [],
...
function boltRefreshSmartProjectContext(reason, forceMainDetection) {
    ...
    try {
        // A user-selected Hero is authoritative. Analyze, Smart Clean and
        // Organize may refresh workspace context, but they must not silently
        // replace the user's chosen master composition with auto-detection.
        if (state.heroCompManual) {
            output.hero = boltResolveStoredComp("hero");
            if (!output.hero) { state.heroCompManual = false; }
        }
        if (!output.hero) {
    ...
    try {
        log("Smart context [" + safeString(reason) + "]: root=" +
            ...
            (state.heroCompManual ? " | manual hero" : " | auto hero"));
...
function updateRenderQueueLocations(renderFolder, context) {
    ...
    function resolveHeroComp() {
        ...
        // Manual Hero selection has first priority until the user presses Find
        // or the selected composition no longer exists in this project.
        if (state.heroCompManual) {
            var manualHero = boltResolveStoredComp("hero");
            if (manualHero) { return manualHero; }
            state.heroCompManual = false;
        }
    ...
    function lockHeroComp() {
        ...
        state.heroCompManual = true;
        boltStoreCompReference("hero", comp);
        updateHeroLabel();
        setStatus("Hero comp locked manually: " + boltProjectItemName(comp, "Selected comp"), "ok");
        return comp;
    }
...
function updateHeroLabel() {
        ...
        state.ui.heroLabel.text = comp
            ? ("Main comp  •  " + name + (state.heroCompManual ? "  [manual]" : ""))
            : "Main comp  •  none";
        state.ui.heroLabel.helpTip = comp
            ? ("Main composition: " + name + (state.heroCompManual ? "\nManually selected — Organize will keep this Hero." : "\nAutomatically detected."))
            : "No composition could be detected.";
    }

    function boltHeroUsedSceneSet(heroComp) {
        var usedScenes = {}, visited = {};
        if (!boltIsCompItem(heroComp)) { return usedScenes; }

        function walk(comp) {
            var compKey = boltProjectItemKey(comp) || ("name:" + boltProjectItemName(comp, ""));
            if (visited[compKey]) { return; }
            visited[compKey] = true;

            var layerIndex, source, sourceKey;
            for (layerIndex = 1; layerIndex <= comp.numLayers; layerIndex++) {
                try { source = comp.layer(layerIndex).source; }
                catch (ignoreHeroSceneSource) { source = null; }
                if (!boltIsCompItem(source)) { continue; }

                sourceKey = boltProjectItemKey(source) || ("name:" + boltProjectItemName(source, ""));
                if (source !== heroComp && isNamedSceneComp(source)) {
                    usedScenes[sourceKey] = true;
                }
                walk(source);
            }
        }
    ...
    function boltHeroUsesScene(sceneComp, usedSceneSet) {
        if (!boltIsCompItem(sceneComp) || !usedSceneSet) { return false; }
        var key = boltProjectItemKey(sceneComp) || ("name:" + boltProjectItemName(sceneComp, ""));
        return usedSceneSet[key] === true;
    }
    ...
    function organizeProjectPanel(heroComp, context) {
        ...
        var leafItems = [];
        var usedSceneSet = boltHeroUsedSceneSet(heroComp);
        ...
        for (index = 0; index < leafItems.length; index++) {
            ...
            try {
                // Project root is intentionally minimal: the chosen Hero and only
                // named Scene/Shot comps that are actually reachable from that Hero.
                // Unused Scene comps are treated like normal comps and go to 01_Comps.
                if (boltIsCompItem(current) && heroComp && current === heroComp) {
                    if (current.parentFolder !== app.project.rootFolder) {
                        current.parentFolder = app.project.rootFolder;
                        context.projectItemsMoved++;
                    }
                    context.heroCompName = current.name;
                    continue;
                }

                if (boltIsCompItem(current) && isNamedSceneComp(current) && boltHeroUsesScene(current, usedSceneSet)) {
                    if (current.parentFolder !== app.project.rootFolder) {
                        current.parentFolder = app.project.rootFolder;
                        context.projectItemsMoved++;
                    }
...
function organizeProject() {
    ...
    // Respect a manually locked Hero before any automatic main-comp guess.
    var heroComp = state.heroCompManual
        ? boltResolveStoredComp("hero")
        : null;
    if (!heroComp) { heroComp = organizeAudit.heroComp || resolveHeroComp() || detectMainComp(); }
...
function boltProjectCleanRefreshStructure() {
        ...
        try {
            ...
            if (status) {
                ...
                status.text = (hero ? (state.heroCompManual ? "Hero manual" : "Hero ready") : "Hero automatic") + "  •  " + scenes + " used scene comp(s) at root";
            }
    ...
    function boltProjectCleanDestination(item, coreFolders, heroComp, usedSceneSet) {
        var category, file;
        if (!item || boltIsFolderItem(item)) { return null; }
        if (boltIsCompItem(item)) {
            if (heroComp && item === heroComp) { return app.project.rootFolder; }
            if (isNamedSceneComp(item) && boltHeroUsesScene(item, usedSceneSet)) { return app.project.rootFolder; }
    ...
    function boltCleanProjectPanelSafe(context, expressionEntries) {
        ...
        for (index = 0; index < leafItems.length; index++) {
            ...
            try {
                ...
                destination = boltProjectCleanDestination(item, coreFolders, heroComp, usedSceneSet);
    ...
    function boltOpenUpdatePage() {
        var url = BOLT_UPDATE_PAGE_URL;
        if (!url || !url.length) { throw new Error("Bolt update page is not configured."); }

        var isWindows = false;
        try { isWindows = $.os.toLowerCase().indexOf("windows") !== -1; }
        catch (ignoreUpdatePageOS) {}

        try {
            if (isWindows) {
                // Let Windows open the user's default browser. No AE network
                // permission, GitHub API call, download or script replacement.
                system.callSystem('cmd.exe /d /s /c start "" "' + url.replace(/"/g, "") + '"');
            } else {
                system.callSystem('/usr/bin/open ' + shellSingleQuote(url));
            }
            setStatus("Opening official BOLT GitHub page", "ok");
            return true;
        } catch (openUpdatePageError) {
            throw new Error("Could not open the BOLT GitHub page in your browser.");
        }
    }

    function showInfoDialog() {
        var dialog = new Window("dialog", "Bolt " + VERSION + " • Info");
        ...
        dialog.add("statictext", undefined,
            "15.9 production update\r\n\r\n" +
            "• Manual Hero selection is authoritative\r\n" +
            "• Organize keeps only used Scene comps at Project root\r\n" +
            "• Unused Scene comps return to 01_Comps\r\n" +
            "• Safe Smart Clean timing protections retained\r\n" +
            "• Organize + layered PSD/AI safeguards retained\r\n" +
            "• Check Update opens the official GitHub page",
            { multiline: true });

        var updateStatus = dialog.add("statictext", undefined, "Updates • github.com/tasnimul-rsd/BOLT");
        updateStatus.alignment = ["fill", "top"];
        try { updateStatus.graphics.foregroundColor = updateStatus.graphics.newPen(updateStatus.graphics.PenType.SOLID_COLOR, [0.72,0.74,0.77], 1); } catch (ignoreUpdateStatusColor) {}
        ...
        buttonRow.spacing = 6;
        var checkButton = buttonRow.add("button", undefined, "Check Update");
        var closeButton = buttonRow.add("button", undefined, "Close", { name: "ok" });

        checkButton.helpTip = "Open the official BOLT GitHub page in your default browser.";
        checkButton.onClick = function () {
            try { boltOpenUpdatePage(); }
...
function compactButton(parent, textValue, width) {
        ...
        infoButton.helpTip = "About Bolt / Open GitHub updates";
...
function updateNavigationState(panelWidth) {
        ...
        state.ui = {
            ...
            fontPreset:fontPreset,textVisualPreset:textVisualPreset,infoButton:infoButton
        };
        ...
        function refreshProjectStructureList() {
            ...
            projectStructureStatus.text = (hero ? (state.heroCompManual ? "Hero manual" : "Hero ready") : "Hero automatic") + "  •  " + scenes + " used scene comp(s) at root";
        }
